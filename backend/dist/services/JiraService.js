...
class JiraService {
    ...
    async createProject(projectData) {
        try {
            ...
            const team = projectData.assignedTeam ? Team_1.TeamModel.findById(projectData.assignedTeam) : null;
            const jiraProject = {
            ...
            const project = Project_1.ProjectModel.findById(projectData.id);
            if (project) {
                Project_1.ProjectModel.update(projectData.id, {
    ...
    async updateProject(projectId, updates) {
        try {
            ...
            const jiraResult = await response.json();
            Project_1.ProjectModel.update(projectId, {
    ...
    async syncDeviceExpiry(deviceId) {
        try {
            const device = Device_1.DeviceModel.findById(deviceId);
            if (!device) {
                throw new Error('Device not found');
            }
            const project = Project_1.ProjectModel.getByDevice(deviceId)[0];
            if (!project) {
