...
exports.ServiceNowService = void 0;
...
class ServiceNowService {
    ...
    async syncTimeEntries(teamId) {
        try {
            const team = Team_1.TeamModel.findById(teamId);
            if (!team) {
                throw new Error('Team not found');
            }
            const users = User_1.UserModel.findAll().filter(user => user.teamId === teamId);
            const timeEntries = TimeEntry_1.TimeEntryModel.findAll().filter(entry => entry.teamId === teamId);
    ...
    async createForecast(forecastData) {
        try {
            ...
            const users = User_1.UserModel.findAll().filter(user => user.teamId === forecastData.teamId);
            const timeEntries = TimeEntry_1.TimeEntryModel.findAll().filter(entry => entry.teamId === forecastData.teamId);
            ...
            const result = await response.json();
            Forecast_1.ForecastModel.update(forecastData.id, {
    ...
    async calculateTeamCapacity(teamId) {
        try {
            const team = Team_1.TeamModel.findById(teamId);
            if (!team) {
                throw new Error('Team not found');
...
exports.ServiceNowService = ServiceNowService;
//# sourceMappingURL=ServiceNowService.js.map